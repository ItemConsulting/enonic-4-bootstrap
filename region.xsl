...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" 
    ...
    <xsl:template name="bootstrap:region.create" as="element()*">
        <xsl:param name="layout" as="xs:string" select="'default'"/>
        <xsl:apply-templates select="$stk:theme-device-class/layout[tokenize(@name, ',') = $layout]/row" mode="bootstrap:region.create"/>
    </xsl:template>
    
    <xsl:template match="row" mode="bootstrap:region.create">
        <div class="{concat('row ', @class)}">
            <xsl:apply-templates select="group|region[@name = $stk:region.active-regions/name]" mode="bootstrap:region.create"/>
        </div>
    </xsl:template>

    <xsl:template match="group" mode="bootstrap:region.create">
        <div>
            <xsl:attribute name="class">
                <xsl:text>group</xsl:text>
                <xsl:value-of select="concat(concat(' col-lg-', @cols), concat(' col-md-', @cols))"/>
            </xsl:attribute>
            <xsl:apply-templates select="region[@name = $stk:region.active-regions/name]" mode="bootstrap:region.create"/>
        </div>
    </xsl:template>

    <xsl:template match="region" mode="bootstrap:region.create">
        ...
        <xsl:element name="{if (current()/@element) then current()/@element else 'div'}">
            <xsl:attribute name="class">
                ...
                <xsl:value-of select="concat(' col-lg-', $cols)"/>
                <xsl:value-of select="concat(' col-md-', $cols)"/>
                <text> col-sm-12 col-xs-12 </text>
